...
function ChatScreen({ auth, onLogout }: { auth: AuthData; onLogout: ()=>void }) {
  ...
  // Swipe-left/menu delete is a per-user hide, not a global delete.
  // Browser confirm/alert is avoided; confirmation is handled by the centered app modal.
  const requestDeleteConversation=(cid:number)=>{setOpenMenuConversationId(null);setDeleteTargetId(cid)}
  const deleteConversation=async(cid:number)=>{const r=await fetch(`${API_BASE}/conversations/${cid}?userId=${auth.userId}`,{method:'DELETE',headers:authHeaders()});const j:ApiResp<boolean>=await r.json();if(!j.success)return setInfo(j.message||'Không xóa được đoạn chat');setConversations(prev=>prev.filter(c=>c.id!==cid));if(selectedId===cid){setSelectedId(null);setMessages([]);localStorage.removeItem('chat_selected_conversation_id')}setSwipedConversationId(null);setOpenMenuConversationId(null);setDeleteTargetId(null)}
  ...
  return <div className={`chat-layout ${selected?'chat-open':'home-open'}`}>
    ...
    <aside className={`left ${mobileSidebarOpen?'open':''}`}>
      ...
      <div className='users-panel chat-list-scroll'>
        ...
        {filteredConversations.map(c=><div key={c.id} className={`${swipedConversationId===c.id?'chat-swipe open':'chat-swipe'} ${openMenuConversationId===c.id?'menu-open':''}`}><button type='button' className='delete-chat-btn' onClick={()=>requestDeleteConversation(c.id)}>🗑</button><div className={`${selectedId===c.id?'contact-row clickable active':'contact-row clickable'} ${(c.unreadCount||0)>0?'unread':''}`} onTouchStart={(e)=>{e.currentTarget.dataset.sx=String(e.touches[0].clientX)}} onTouchEnd={(e)=>{const sx=Number(e.currentTarget.dataset.sx||0);const dx=e.changedTouches[0].clientX-sx;if(dx < -45)setSwipedConversationId(c.id);else if(dx > 35)setSwipedConversationId(null)}} onClick={()=>{if(swipedConversationId===c.id){setSwipedConversationId(null);return}setOpenMenuConversationId(null);setSelectedId(c.id);setMobileSidebarOpen(false)}}><div className='contact-left'><div className='avatar'>{displayTitle(c)[0]?.toUpperCase()||'C'}</div><div className='meta'><div className='name'>{displayTitle(c)} {c.type==='DIRECT'&&<span className='dot'/>}</div><div className='desc'>{c.type==='GROUP'?'Nhóm':'Đang hoạt động'} • {c.lastMessageAt?fmtVNTime(c.lastMessageAt):'mới'}</div></div></div><div className='conversation-actions'>{!!c.unreadCount&&<span className='badge'>{c.unreadCount}</span>}<button type='button' className='more-chat-btn' aria-label='Mở menu đoạn chat' onClick={(e)=>{e.stopPropagation();setSwipedConversationId(null);setOpenMenuConversationId(v=>v===c.id?null:c.id)}}>⋯</button>{openMenuConversationId===c.id&&<div className='chat-menu' onClick={(e)=>e.stopPropagation()}><button type='button' onClick={()=>requestDeleteConversation(c.id)}>Xóa đoạn chat</button></div>}</div></div></div>)}
      </div>
    </aside>
    {deleteTargetId!==null&&<div className='modal-backdrop' onClick={()=>setDeleteTargetId(null)}><div className='confirm-modal' onClick={e=>e.stopPropagation()}><div className='confirm-icon'>!</div><h3>Xóa đoạn chat?</h3><p>Đoạn chat sẽ chỉ bị ẩn khỏi danh sách của bạn. Người khác vẫn giữ lịch sử của họ.</p><div className='confirm-actions'><button type='button' className='confirm-cancel' onClick={()=>setDeleteTargetId(null)}>Hủy</button><button type='button' className='confirm-danger' onClick={()=>deleteConversation(deleteTargetId)}>Xóa đoạn chat</button></div></div></div>}
